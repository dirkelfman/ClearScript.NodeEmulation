var builtinModules =(function() {
    ...
    EventEmitter.prototype.on = function(event, listner) {
        return this.ccInner.on(event, listner);
    };
    ...
    Request.prototype.on = function(event, callback) {
        if (event == 'response') {
            this.ccInner.on(event, function(ccInner) {
                var incommingMessage = new IncommingMessage(ccInner);
                callback(incommingMessage);
            });
        } else {
            this.ccInner.on(event, callback);
        }
        return this;
